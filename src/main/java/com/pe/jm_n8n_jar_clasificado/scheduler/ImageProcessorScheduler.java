...
public class ImageProcessorScheduler {
     ...
     * Processes the oldest valid .jpg file from the input folder.
     */
    @Scheduled(cron = "0 * * * * *")
    public void processNextImage() {
        ...
        try {
            ...
            boolean uploadSuccess = uploadService.uploadFile(file);

            if (uploadSuccess) {
                log.info("Upload successful for file: {}", file.getName());
                postProcessService.processSuccessfulUpload(file);
            } else {
                log.error("Upload failed for file: {}", file.getName());
                postProcessService.processFailedUpload(file);
            }
